...
getLatestVersion() {
    ...
    LATEST_VERSION=$(curl -s https://api.github.com/repos/Gu1llaum-3/gosshm/releases/latest | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
...
downloadBinary() {
    ...
    GITHUB_URL="https://github.com/Gu1llaum-3/gosshm/releases/download/$LATEST_VERSION/$GITHUB_FILE"
